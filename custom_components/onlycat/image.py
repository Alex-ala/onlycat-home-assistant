...
class OnlyCatLastImage(ImageEntity):
    ...
    def __init__(
        ...
        self._attr_unique_id = device.device_id.replace("-", "_").lower() + "_last_activity_image"
    ...
    async def on_event_update(self, data: dict) -> None:
        ...
        _LOGGER.debug("Processing event update for image:  %s: %s", self.device.device_id, str(data))
        ...
        frame_to_show = self._current_event.poster_frame_index \
            if self._current_event.poster_frame_index is not None else \
            self._current_event.frame_count / 2 if self._current_event.frame_count is not None else 1
        self._attr_image_url = IMAGE_BASEURL + \
    ...
    async def update_event(self, event: Event) -> None:
        ...
        _LOGGER.debug("Updating event for device %s: %s", self.device.device_id, str(event))
